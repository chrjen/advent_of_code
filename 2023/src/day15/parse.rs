...
pub(super) fn step_dash(input: &str) -> IResult<&str, Step<'_>> {
    map(terminated(alpha1, complete::char('-')), |label: &str| {
        Step::new(label, Operation::Dash)
    })(input)
}

pub(super) fn step_equal(input: &str) -> IResult<&str, Step<'_>> {
    map(
...
pub(super) fn parse_steps(input: &str) -> IResult<&str, Box<[Step<'_>]>> {
    map(
